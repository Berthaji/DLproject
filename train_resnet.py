...
def train_model_resnet(model, train_loader, val_loader, criterion, optimizer, num_epochs=10, output_path='results/training_results_resnet.csv', device="cpu", validate=True):
    """
    Funzione per addestrare un modello ResNet, registrare i risultati e salvarli in un file CSV.
    Questo esempio allena gli ultimi due layers (l'ultimo fully connected e il penultimo layer convoluzionale).
    ...
    Args:
        ...
        output_path: Percorso per salvare i risultati (default: 'results/training_results_resnet.csv').
        device: Dispositivo per l'addestramento (default: "cpu").
    """
    # Congelare i layers non necessari per il transfer learning
